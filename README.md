...
rers = readRDS("rer_mammal108.rds")
```
...
```
corEnamel = correlateWithBinaryPhenotype(rers, phenoEnamel, min.sp=10, min.pos=2, weighted="auto", winsorizeRER=2 )
...
```{r, message = FALSE, cache = TRUE}
# If RStudio
View(corEnamel)  # then sort by stat
...
5. The second and third ranked genes are olfactory receptors (ORs). Why do you think they would be accelerated in these species?
